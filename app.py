...
with tab2:
    ...
    if "DM_Response_Rate" in df.columns:
        ...
        df["AI_Rank"] = pd.cut(df["AI_Score"], bins=[0, 0.3, 0.6, 1.0], labels=["C: Skip", "B: Maybe", "A: Send"])
        st.divider()
...
with tab4:
    ...
    avg_resp_all = df["DM_Response_Rate"].mean() if "DM_Response_Rate" in df.columns else 0.03
    avg_resp_ai = df.nlargest(max_sends, "AI_Score")["DM_Response_Rate"].mean() if "AI_Score" in df.columns and "DM_Response_Rate" in df.columns else avg_resp_all * 1.5
    resp_all = int(total_customers * avg_resp_all)
    resp_ai = int(max_sends * avg_resp_ai)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("### Send to ALL")
        st.metric("Send Count", f"{total_customers:,}")
        st.metric("Total Cost", f"¥{cost_all:,}")
        st.metric("Expected Responses", f"{resp_all:,}")
        st.metric("Cost per Response", f"¥{cost_all // max(resp_all,1):,}")
    with c2:
        st.markdown("### AI-Optimized")
        st.metric("Send Count", f"{max_sends:,}")
        st.metric("Total Cost", f"¥{cost_ai:,}")
        st.metric("Expected Responses", f"{resp_ai:,}")
        st.metric("Cost per Response", f"¥{cost_ai // max(resp_ai,1):,}")
    st.divider()
    s1, s2, s3 = st.columns(3)
    s1.metric("Savings per Campaign", f"¥{savings:,}")
    s2.metric("Annual Savings (12x)", f"¥{savings*12:,}")
    roi = (savings / max(cost_ai, 1)) * 100
    s3.metric("ROI Improvement", f"{roi:.0f}%")
    fig_comp = go.Figure(data=[
        go.Bar(name="Send All", x=["Cost", "Responses"], y=[cost_all, resp_all]),
        go.Bar(name="AI-Optimized", x=["Cost", "Responses"], y=[cost_ai, resp_ai])
    ])
    fig_comp.update_layout(barmode="group", title="Cost vs Response Comparison")
...
with tab6:
    ...
    **AI learns, so protect it.**
    ...
    |---|---|---|
    | Auto-backup | Every re-training | Automatic (saved to ~/DM_AI_Backup/) |
    | USB backup | Monthly | Click "Create Backup ZIP" → save to USB |
    | Shared folder | Weekly | Copy ~/DM_AI_Backup/ folder |
    | PC replacement | When needed | Upload .pkl file to restore |
    """)

st.divider()
st.caption("📮 DM AI Optimizer v2.0 | Backup Enabled | Powered by Streamlit + scikit-learn")
